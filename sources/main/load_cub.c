...
static void	set_up_cub(t_cub *cub)
{
	...
	cub->camera.fov_rad = FOV;
	cub->camera.dir_x = cos(cub->player.angle);
	cub->camera.dir_y = sin(cub->player.angle);
	cub->camera.plane_x = -cub->camera.dir_y * tan(cub->camera.fov_rad / 2);
	cub->camera.plane_y = cub->camera.dir_x * tan(cub->camera.fov_rad / 2);
